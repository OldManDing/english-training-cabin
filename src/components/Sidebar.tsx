...
import { BarChart3, BookOpen, ChevronLeft, ChevronRight, ClipboardCheck, DownloadCloud, FolderSync, HelpCircle, Mic, Settings, Sparkles } from 'lucide-react';
...
export default function Sidebar({ activeTab, setActiveTab, onTriggerModal }: SidebarProps) {
  ...
  const updateScrollControls = useCallback(() => {
    ...
    if (!nav) return;
    setCanScrollPrevious(nav.scrollLeft > 4);
    setCanScrollNext(nav.scrollLeft + nav.clientWidth < nav.scrollWidth - 4);
  }, []);
  ...
  const openHelp = () => {
    ...
    onTriggerModal?.(
      '使用帮助',
      '英语训练舱当前聚焦 CET-4 首发场景，并按多考试训练系统预留架构。\n\n1. 点击今日标题旁的“入门能力诊断”建立初始能力画像。\n\n2. 使用主导航在专项练习、阶段模考、复习队列、口语重说和能力进展间切换；已答题目可在专项练习中回看。\n\n学习记录会自动保存到服务端，当前浏览器 IndexedDB 只作为离线副本；启用 AI 分析时会发送必要文本用于生成反馈。',
    );
  };

  const scrollPrimaryNav = (direction: -1 | 1) => {
    primaryNavRef.current?.scrollBy({ left: direction * 248, behavior: 'smooth' });
  };
  ...
  }, [activeTab, updateScrollControls]);

  useEffect(() => {
    const nav = primaryNavRef.current;
    if (!nav) return;
    updateScrollControls();
    nav.addEventListener('scroll', updateScrollControls, { passive: true });
    window.addEventListener('resize', updateScrollControls);
    return () => {
      nav.removeEventListener('scroll', updateScrollControls);
  ...
  return (
    <aside className="app-page-surface ui-sidebar sticky top-0 z-40 flex h-auto w-full shrink-0 flex-col border-b lg:h-screen lg:w-64 lg:justify-between lg:border-b-0 lg:border-r select-none">
      <div className="flex flex-col">
        <div className="flex items-center gap-3 p-3 lg:px-6 lg:pb-5 lg:pt-6">
          ...
          <div className="ml-auto flex items-center gap-1 lg:hidden">
            <button
              ...
              aria-label="设置"
              title="设置"
              className={`ui-button ui-button-icon ${activeTab === 'settings' ? 'ui-sidebar-item-active' : ''}`}
            >
        ...
        <div className="flex items-center lg:block">
          <button
            type="button"
            data-testid="mobile-primary-nav-previous"
            onClick={() => scrollPrimaryNav(-1)}
            disabled={!canScrollPrevious}
            aria-label="查看前面的主导航"
            title="前面的主导航"
            className={`ui-button ui-button-icon ml-1 shrink-0 border-slate-200 bg-white lg:hidden ${canScrollPrevious ? '' : 'invisible'}`}
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <nav
            ref={primaryNavRef}
            className="flex min-w-0 flex-1 gap-2 overflow-x-auto overscroll-x-contain px-1 pb-3 lg:block lg:space-y-1 lg:p-3"
          >
            {menuItems.map((item) => {
          ...
          <button
            type="button"
            data-testid="mobile-primary-nav-next"
            onClick={() => scrollPrimaryNav(1)}
            disabled={!canScrollNext}
            aria-label="查看更多主导航"
            title="更多主导航"
            className={`ui-button ui-button-icon mr-1 shrink-0 border-slate-200 bg-white lg:hidden ${canScrollNext ? '' : 'invisible'}`}
          >
      ...
      <div className="hidden px-3 pb-3 lg:block lg:p-3 lg:pb-6">
