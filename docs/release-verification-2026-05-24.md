...
> 2026-05-26 产品边界调整：邮件交付、邮箱验证和邮件找回密码已移出当前公开能力；团队邀请改为手动分享链接。本文原有邮件相关验收项仅记录历史实现。
...
- 上线说明：产品内新增本地数据、版权来源、AI 辅助可降级声明入口。
- 观测能力：新增 `/api/telemetry/event` 和 `/api/observability/summary`，统计 API 错误率、反馈兜底率、耗时和关键产品事件。
...
- 产品精修：对外名称统一为“英语训练舱 / 英语能力训练系统”，减少单一 CET-4 AI 教练表述。
...
- SaaS 第四批：不继续付费方向，移除邮件交付上线要求，补齐设备会话列表、团队邀请链接、团队管理 UI、内容授权治理、数据权利请求和 owner-only 运营观测。
- SaaS 数据库：新增 `migrations/0003_commercial_ops.sql`，扩展 session 设备元数据、内容资产和数据请求表。
- SaaS 前端：设置页新增“商业化 SaaS 操作台”，覆盖团队邀请链接、设备撤销、内容资产登记、本人云端档案下载、数据导出/删除请求和运营概览。
...
| 产品声明可见 | 移动端 E2E | 本地数据、版权来源、离线兜底入口在首屏可见且可打开 |
...
| SaaS 第二批商业化能力 | API 测试 | 订阅 webhook 签名、订阅幂等、增量实体同步和跨租户隔离通过；邮箱验证和密码重置保留为历史能力，不纳入当前发布口径 |
| SaaS 协作与会话治理 | API 测试 | token 刷新、登出撤销、团队邀请、邀请一次性消费、成员列表、owner-only 管理概览通过 |
| SaaS 非付费商业化操作台 | API + E2E | 设备会话、团队邀请链接、内容授权、数据权利请求和运营观测均可从设置页操作 |
...
- 通过：SaaS 第四阶段非付费商业化能力覆盖设备会话列表、团队邀请链接、团队管理 UI、内容授权状态、数据权利请求和运营观测；邮件交付不作为当前产品能力。

## 产品验收结论

- 可以作为本地优先英语训练产品上线试用：首屏目标清晰，今日训练、专项练习、复习队列、口语重说、能力进展、材料导入、设置与备份恢复形成闭环。
- 对外定位已从单点 “CET-4 AI 备考训练舱” 修正为“英语训练舱 / 英语能力训练系统”，首发仍聚焦 CET-4，底层保留多考试扩展。
- 版权、隐私、本地数据、AI 辅助可降级与供应商失败风险已在产品内显式提示，避免用户误以为是官方真题或云端账号产品。
...
- SaaS 非付费商业化边界：团队管理、内容授权、运营观测和合规请求已经可操作；若公开上线，需要在真实部署环境配置真实 Postgres、AI Key 和正式法务文本。
