...
> 2026-05-26 产品边界调整：当前版本不启用邮件交付、邮箱验证或邮件找回密码；团队邀请统一通过可复制链接完成。下文邮件相关内容仅保留为历史迭代记录，不再作为上线要求。
...
- 账号标识边界：邮箱仅作为登录账号标识，当前版本不启用邮件交付、邮箱验证或邮件找回密码。
- 团队邀请入口：团队邀请通过可复制链接完成，不依赖开发 token、邮件服务或 API 手工调用。
...
| 邮件交付 | 不纳入当前产品 | 邮箱只作为账号标识；邀请通过可复制链接完成 |
